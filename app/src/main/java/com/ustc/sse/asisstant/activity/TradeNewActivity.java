...
public class TradeNewActivity extends AppCompatActivity {
    private int m_id;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.trade_forum_page);
        ...
        m_kxReward=(EditText)this.findViewById(R.id.newThreadKxRewardtr);
        m_subject = (EditText)this.findViewById(R.id.newThreadSubjecttr);
                ...
                switch (msg.arg1) {
                    case 1:
                        ...
                        if (flag!=null&&flag.equals("success"))
                            Toast.makeText(TradeNewActivity.this,"发帖成功", Toast.LENGTH_LONG).show();
                        else
                            Toast.makeText(TradeNewActivity.this,"发帖失败", Toast.LENGTH_LONG).show();
                        break;
                    default:
                        break;
                }
        ...
        Spinner spinner = (Spinner)this.findViewById(R.id.newThreadTopicSelecttr);
        final String[] topics = getResources().getStringArray(R.array.trade_list);
    ...
    public void onBackPressed() {
        ...
        Intent intent = TradeNewActivity.this.getIntent();
        intent.putExtras(data);
        TradeNewActivity.this.setResult(1, intent);
        TradeNewActivity.this.finish();
    }
    ...
    public void onSubmitBtnClick(View v) {
        EditText message = (EditText)this.findViewById(R.id.newThreadMessagetr);
        final String subjectText = m_subject.getText().toString();
        String msgText = message.getText().toString();

        if (m_subject.length() == 0) {
            Toast.makeText(TradeNewActivity.this, "标题不能为空", Toast.LENGTH_SHORT).show();
        ...
        if (m_currentTopic == 0) {
            Toast.makeText(TradeNewActivity.this, "请选择话题", Toast.LENGTH_SHORT).show();
            return;
        }

        if (message.length() == 0) {
            Toast.makeText(TradeNewActivity.this, "内容不能为空", Toast.LENGTH_SHORT).show();
            message.requestFocus();
            return;
        } else if (message.length() < 6) {
            Toast.makeText(TradeNewActivity.this, "内容长度不能小于" + 6 + "个字符", Toast.LENGTH_SHORT).show();
            message.requestFocus();
            return;
        }
        new Thread(){
           ...
           //         json.put("bbs_id",m_id);
                    ...
                    new GetDirectorBBS(m_handler).getData(GetDirectorBBS.TradeInsert,send);
